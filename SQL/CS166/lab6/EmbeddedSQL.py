...
def query1(esql):
    ...
    try:
        row_count = esql.execute_query(
            "SELECT s.sname, COUNT(*) AS num_parts "
            "FROM suppliers s "
...
def query3(esql):
    ...
    try:
        row_count = esql.execute_query(
            ...
            "GROUP BY s.sname "
            "HAVING COUNT(*) = COUNT(*) FILTER (WHERE LOWER(TRIM(p.color)) = 'green');"
        )
        print(f"total row(s): {row_count}")
    except Exception as e:
        print(e, file=sys.stderr)
...
# For every supplier that supplies green part and red part, print the name and the price of the most expensive part that they supply
def query4(esql):
    # Your code goes here.
    try:
        row_count = esql.execute_query(
            "SELECT s.sname, MAX(cat.cost) AS max_cost "
            "FROM suppliers s "
            "JOIN catalog cat ON s.sid = cat.sid "
            "JOIN parts p ON cat.pid = p.pid "
            "GROUP BY s.sname "
            "HAVING COUNT(*) FILTER (WHERE LOWER(TRIM(p.color)) = 'green') > 0 "
            "AND COUNT(*) FILTER (WHERE LOWER(TRIM(p.color)) = 'red') > 0;"
        )
        print(f"total row(s): {row_count}")
    except Exception as e:
        print(e, file=sys.stderr)
...
# Find the name of parts with cost lower than a user-supplied value
def query5(esql):
    # Your code goes here.
    try:
        cost = input("\tEnter cost: $")
        row_count = esql.execute_query(
            "SELECT p.pname "
            "FROM parts p "
            "JOIN catalog cat ON p.pid = cat.pid "
            "WHERE cat.cost < %s;",
            (cost,)
        )
        print(f"total row(s): {row_count}")
...
# Find the address of the suppliers who supply a user-supplied part name (pname)
def query6(esql):
    # Your code goes here.
    try:
